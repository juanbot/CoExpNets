...
saveRDS(rpkms,"~/coexp/examples/rosmap_rpkms.rds")
```
...
library(swamp)
#We have to prepare everything to be reading for using the code
source("~/Dropbox/clases/bioinf2017/tia1718/tutorialesR/coexp/coexpression.R",chdir=T)
#We initialise already available networks (only ROS/MAP this time)
coexp.initDb("NoFull")

#Load the selected genes and samples from the file produced with the example code from above
rpkms = readRDS(paste0(gdp.coexp(),"/examples/rosmap_rpkms.rds"))
...
sampdist = as.matrix(dist(coexp.trasposeDataFrame(rpkms,F)))
...
if(result$p.value < 0.05){
  ...
  plot(flashClust(dist(coexp.trasposeDataFrame(rpkms,F)), method = "average"),cex=0.5,
...
#Normalize samples
rpkms_qn = normalize.quantiles(as.matrix(rpkms))
...
```{r cache=TRUE}
rpkms_combat = ComBat(dat=rpkms_qn,batch=covs$batch,
...
print(Sys.time())
svas = svaseq(dat=as.matrix(rpkms_combat),mod=mm,mod0=nullmm)
print(Sys.time())
...
```{r}
saveRDS(resids,paste0(gdp.coexp(),"/examples/rosmap_residuals.rds"))
```
